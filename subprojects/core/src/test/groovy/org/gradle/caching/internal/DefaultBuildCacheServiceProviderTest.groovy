...
class DefaultBuildCacheServiceProviderTest extends Specification {
    ...
    private buildCacheConfiguration = Stub(BuildCacheConfigurationInternal) {
        getFactory(LocalBuildCache) >> localBuildCacheFactory
        getFactory(RemoteBuildCache) >> remoteBuildCacheFactory
    ...
    private DefaultBuildCacheServiceProvider provider = new DefaultBuildCacheServiceProvider(buildCacheConfiguration, startParameter, buildOperationExecuter)
    ...
    def 'fails when local and remote have push enabled'() {
        ...
        e.message == 'It is only allowed to push to a remote or a local cache, not to both.'
    }

    private class RemoteBuildCache extends AbstractBuildCache {}
}
